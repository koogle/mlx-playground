...
class SpeechCommandsDataset:
        ...
        if self.overfit_mode:
            ...
            if len(available_classes) >= self.overfit_samples:
                ...
                for i in range(self.overfit_samples):
                    ...
                    path, label = samples_by_class[class_label][
                        0
                ...
                print(
                    f"Overfit mode: Selected {len(self.file_paths)} samples from different classes: {selected_classes}"
                )
            else:
                ...
                selected_classes = [self.classes[label] for label in self.labels]
                print(
                    f"Overfit mode: Limited to {len(self.file_paths)} files: {selected_classes}"
                )
        else:
    ...
    def _load_classes(self) -> Tuple[List[str], Dict[str, int]]:
        ...
        for item in os.listdir(self.data_dir):
            ...
            if item_path.is_dir() and not item.startswith("_") and item != "LICENSE":
                classes.append(item)
    ...
    def _add_background_noise(self, audio: np.ndarray) -> np.ndarray:
        ...
        if (
            random.random() > self.background_noise_prob
            or not self.background_noise_files
        ):
            return audio
    ...
    def _audio_to_spectrogram(self, audio: np.ndarray) -> np.ndarray:
        ...
        padded_audio = np.pad(
            audio, (self.n_fft // 2, self.n_fft // 2), mode="constant"
        )
