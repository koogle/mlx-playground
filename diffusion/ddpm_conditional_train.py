...
def sample_images_conditional(model, scheduler, class_labels=None, num_samples=4, 
...
def save_conditional_samples(samples, labels, epoch, save_dir="./samples"):
    ...
    print(f"  Classes: {[CIFAR10_CLASSES[l] for l in labels_np[:min(10, len(labels_np))]]}")
    print(f"  Saved to {grid_path}")


def train_epoch(model, scheduler, optimizer, train_loader, epoch, 
                unconditional_prob=0.1, sample_every=50, loss_history=None):
    """
    ...
    for batch_idx, (images, labels) in enumerate(train_loader):
        ...
        loss = train_step(model, scheduler, optimizer, images, labels, unconditional_prob)
        loss_val = loss.item()
        total_loss += loss_val
        num_batches += 1
        ...
        if batch_idx % sample_every == 0 and batch_idx > 0:
            ...
            # Save samples
            save_conditional_samples(all_samples, all_labels, 
...
def main():
    ...
    for epoch in range(start_epoch, config["num_epochs"]):
        ...
        all_samples = mx.concatenate(samples_list, axis=0)
        all_labels = mx.concatenate(labels_list, axis=0)
        save_conditional_samples(all_samples, all_labels, epoch + 1, config["sample_dir"])
