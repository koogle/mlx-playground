...
def get_checkpoint_name(epoch, model_type="model"):
    """
    Generate consistent checkpoint naming across save and load operations

    Args:
        epoch: Epoch number
        model_type: Model type (e.g. "flow_matching", "diffusion", "conditional", etc.)

    Returns:
        Base checkpoint name (without extension)
    """
    return f"{model_type}_checkpoint_epoch_{epoch}"
...
def parse_checkpoint_name(checkpoint_name):
    ...
    if checkpoint_name.startswith("checkpoint_epoch_"):
        try:
            ...
            return epoch, "model"  # Default generic type
        except:
            pass
...
def save_checkpoint(
    model, optimizer, epoch, loss, save_dir, model_type="model", config=None
):
    """
    ...
    # Add model-specific metadata based on model attributes
    if hasattr(model, "num_classes") and model.num_classes is not None:
        metadata["num_classes"] = model.num_classes
    if hasattr(model, "class_emb_dim") and model.class_emb_dim is not None:
        metadata["class_emb_dim"] = model.class_emb_dim
...
def load_checkpoint(model, optimizer, checkpoint_path, expected_type=None):
    ...
    print(f"Loading {metadata.get('model_type', 'unknown')} model from epoch {epoch}")
...
def create_interrupt_handler(
    ...
    Args:
        ...
        model_type: Type of model (e.g. "flow_matching", "diffusion", etc.)
