...
def yolo_loss(predictions, targets, lambda_coord=5.0, lambda_noobj=0.5):
    ...
    ious = mx.stack([compute_iou(pred_boxes[..., i, :4], target_boxes) 
                     for i in range(B)], axis=-1)
    
    # Find best box for each target
    best_box_ious = mx.max(ious, axis=-1, keepdims=True)
    best_box_mask = ious >= best_box_ious
    
    # Coordinate loss (only for cells with objects)
    coord_mask = mx.repeat(target_obj, B, axis=-1) * best_box_mask
    coord_loss = (
        lambda_coord * coord_mask * (
            (pred_boxes[..., 0] - target_boxes[..., 0])**2 +
            (pred_boxes[..., 1] - target_boxes[..., 1])**2 +
            (mx.sqrt(pred_boxes[..., 2]) - mx.sqrt(target_boxes[..., 2]))**2 +
            (mx.sqrt(pred_boxes[..., 3]) - mx.sqrt(target_boxes[..., 3]))**2
        )
    )
    
    # Confidence loss
    conf_mask = mx.repeat(target_obj, B, axis=-1)  # Mask for cells with objects
    noobj_mask = 1 - conf_mask  # Mask for cells without objects
    
    conf_loss = (
        conf_mask * (pred_boxes[..., 4] - ious)**2 +
        lambda_noobj * noobj_mask * (pred_boxes[..., 4] - 0)**2
    )
    
    # Class loss (only for cells with objects)
    class_loss = target_obj * mx.sum((pred_classes - target_classes)**2, axis=-1)
    
    # Total loss
    total_loss = mx.mean(mx.sum(coord_loss + conf_loss, axis=-1) + class_loss)
